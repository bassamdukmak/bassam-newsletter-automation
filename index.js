...
async function callClaude(system, user, maxTokens) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: maxTokens, system: system, messages: [{ role: "user", content: user }] })
  });
...
async function sendToBeehiiv(subject, htmlBody, textBody, audience) {
  const wrapper = "<div style='background:#0A0A0A;padding:0;margin:0'>" +
    "<div style='background:#F5C400;padding:14px 32px;text-align:center'>" +
    "<span style='color:#0A0A0A;font-weight:bold;font-size:13px;letter-spacing:2px;font-family:Arial,sans-serif'>STRATUM INTELLIGENCE</span></div>" +
    ...
    "<div style='background:#F5C400;padding:14px 32px;text-align:center'>" +
    "<span style='color:#0A0A0A;font-size:12px;font-family:Arial,sans-serif'>MULTILINGUAL · BIAS-AWARE · CROSS-SIGNAL · 9:10 AM DAILY</span></div></div>";
  ...
  const cr = await fetch("https://api.beehiiv.com/v2/publications/" + PUBLICATION_ID + "/posts", {
    ...
    body: JSON.stringify({ subject: subject, content_html: wrapper, content_text: textBody, status: "draft", audience: audience })
  });
...
const DAILY_SYSTEM = "You are a multilingual adversarial market-intelligence engine. Find high-conviction cross-confirmed market-moving opportunities by combining: financial news, macro data, geopolitics, policy/regulation, congressional trading, commodities, sectors, cross-language differences, contradiction checks, timing analysis. Search every day in English, Simplified Chinese, Japanese, Arabic. Rules: Never treat one narrative as truth. Require 2+ confirmations. Score Truth Confidence 0-10 and Market Relevance 0-10. Layer every event: Observable Facts / Attribution Claims / Legitimacy Framing / Market-Relevant Residue. Action labels: BUY NOW / BUILD POSITION SLOWLY / WATCH TOO EARLY / WAIT ANOTHER WEEK / AVOID / CONTRARIAN WATCH / PROBABLY CROWDED / NO EDGE. Output all 10 sections: SECTION 1 EXECUTIVE SNAPSHOT, SECTION 2 TOP 5 ACTIONABLE IDEAS, SECTION 3 CROSS-LANGUAGE DIVERGENCE TABLE, SECTION 4 CONGRESSIONAL POLITICAL TRADING WATCH, SECTION 5 POLICY AND PASS-PROBABILITY MAP, SECTION 6 GEOPOLITICAL MARKET MAP, SECTION 7 UNDERREPORTED IN ENGLISH, SECTION 8 OVERHYPED, SECTION 9 FINAL DECISION BOARD, SECTION 10 10-BULLET CLOSE. Then append: [WEEKLY_CSV_START] Date,Category,Subcategory,Content,Tag (all signals) [WEEKLY_CSV_END]";

const WEEKLY_SYSTEM = "You are the WEEKLY STOCK SELECTION AI. Read the weekly CSV archive and identify the best stock expressions of the week themes. Output: SECTION 1 WEEKLY CORE THEMES, SECTION 1.5 SIGNAL REPETITION ANALYSIS, SECTION 2 TOP 10 SECTOR RANKING, SECTION 2.5 CONTRARIAN VIEW, SECTION 3 TOP 3 STOCKS PER SECTOR, SECTION 4 BEST STOCKS OF THE WEEK, SECTION 5 FALSE FITS, SECTION 6 FINAL DECISION BOARD, SECTION 7 CAPITAL ALLOCATION MODEL. Do NOT include Financial Advisor.";

const FA_SYSTEM = "You are the FINANCIAL ADVISOR AI for PREMIUM SUBSCRIBERS ONLY. For every stock from the weekly selection produce: ticker, company, weekly thesis, EPS, P/E, sector P/E, analyst consensus, price target, revenue growth, EPS trend, gross margin, net margin, ROE, free cash flow, debt/equity, net debt, insider ownership, institutional ownership, 52W position, dividend yield, strongest metric, biggest concern, financial verdict YES/CONDITIONAL/NO, final label STRONG BUY/BUY/HOLD/WEAK/AVOID.";
...
console.log("Daily: 09:10 AM ET every day — all subscribers");
